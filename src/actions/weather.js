...
export const getSearchLocations = data => {
    return async () => {
      const res = await api(`https://dataservice.accuweather.com/locations/v1/cities/autocomplete?apikey=${process.env.REACT_APP_APIKEY}&q=${data}`);
...
export const getCurrentCityWeather = data => {
  return async dispatch => {
    const res = await api(`https://dataservice.accuweather.com/currentconditions/v1/${data.getID()}?apikey=${process.env.REACT_APP_APIKEY}`);
...
export const getFiveDaysCityWeather = data => {
  return async dispatch => {
    const res = await api(`https://dataservice.accuweather.com/forecasts/v1/daily/5day/${data.getID()}?apikey=${process.env.REACT_APP_APIKEY}`);
